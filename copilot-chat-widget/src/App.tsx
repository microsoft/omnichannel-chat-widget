...
const App: React.FC<AppProps> = (props: AppProps) => {
  ...
  useEffect(() => {
    if (!props.eventSubscription) return;
    if (!props.eventSubscription.ingressMessageSubject.isStopped()) {
      console.log("debugging: subscribe called");
      try {
        props.eventSubscription.ingressMessageSubject.subscribe((msgArr) => {
  ...
  }, [props.eventSubscription.ingressMessageSubject]);


  useEffect(() => {
    if (props.eventSubscription.attachmentMessageSubject) {
      props.eventSubscription.attachmentMessageSubject.subscribe((val: AttachmentUpdateMessage) => {
        updateAttachmentBlobURL(val, attachmentMessageTracker);
        reloadMessage();
      })
    }
  }, [props.eventSubscription.attachmentMessageSubject])
...
// const attachmentMessageTracker: Map<string, string> = new Map();
  ...
  useEffect(() => {
    if (!props.eventSubscription.flightMessageSubject) return;
    props.eventSubscription.flightMessageSubject.subscribe((val: ChatSDKMessage) => {
  ...
  }, [props.eventSubscription.flightMessageSubject])
