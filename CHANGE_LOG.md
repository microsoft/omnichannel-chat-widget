...
### Fixed
- [Bug] Pre-chat survey required compact `Input.ChoiceSet` no longer fails validation when the **first** choice is selected. `PreChatSurveyPane` previously deleted the placeholder `<option>` as an iOS blank-row workaround, which shifted the first real choice to `selectedIndex` 0. Adaptive Cards' compact ChoiceSet value getter returns `undefined` for `selectedIndex === 0` (it assumes index 0 is the placeholder), so `isRequired` validation failed for the first choice on every platform — not just iOS. The placeholder `<option>` is now preserved; the iOS native-rendering override (`-webkit-appearance`/`height`) is retained.
