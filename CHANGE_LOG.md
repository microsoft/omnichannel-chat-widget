...
### Fixed
- [Bug] Masked customer messages that start with sensitive content (e.g. `1234 is my PIN`) no longer appear trimmed at the start of the chat bubble. When a masking rule matched content at the beginning of a message, the resulting `####` prefix was rendered as a markdown heading by WebChat (because `preProcessingMiddleware` sets `textFormat:"markdown"` on outgoing activities), causing the masked characters to visually disappear. The customer saw a truncated message while the agent saw the full original text. Fix: `dataMaskingMiddleware` now escapes leading `#` characters with a backslash after all masking rules are applied, so markdown renders them as literal characters instead of a heading marker.
